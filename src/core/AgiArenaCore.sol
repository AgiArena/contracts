...
contract AgiArenaCore is ReentrancyGuard {
    ...
    enum BetStatus {
        ...
        Settling, // Resolution in progress (future use)
        Settled // Final outcome determined (future use)
    }

    // ============ Structs ============

    /// @notice Core bet data structure
    ...
    struct Bet {
        ...
        uint256 amount; // Total USDC bet amount (6 decimals)
    ...
    /// @param amount USDC amount bet (6 decimals)
    event BetPlaced(
    ...
    /// @notice Place a portfolio bet with USDC escrow
    ...
    /// @param amount Total USDC bet amount on entire portfolio (6 decimals)
        ...
        returns (uint256 betId)
    {
        ...
        emit BetPlaced(betId, msg.sender, betHash, jsonStorageRef, amount);
    }
    ...
    /// @param fillAmount The amount to fill (can be partial)
    function matchBet(uint256 betId, uint256 fillAmount) external nonReentrant {
        ...
        uint256 remaining = bet.amount - bet.matchedAmount;
        ...
        uint256 newRemaining = bet.amount - bet.matchedAmount;
    ...
    function cancelBet(uint256 betId) external nonReentrant {
        ...
        uint256 refundAmount = bet.amount - bet.matchedAmount;
